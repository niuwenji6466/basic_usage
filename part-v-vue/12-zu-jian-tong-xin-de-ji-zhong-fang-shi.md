...
* 创建一个firstChild组件，引入eventBus这个事件总线，接着添加一个按钮并绑定一个点击事件
  ...
  "userDefinedEvent"
  ,
  "this messgae is from firstChild"
  );
   ...
   # vue组件通信的几种方式
       ...
       - 创建一个firstChild组件，引入eventBus这个事件总线，接着添加一个按钮并绑定一个点击事件
         ...
         ```
            ...
            我们在响应点击事件的sendMsg函数中用$emit触发了一个自定义的userDefinedEvent事件，并传递了一个字符串参数,ps:$emit实例方法触发当前实例(*这里的当前实例就是bus*)上的事件,附加参数都会传给监听器回调。 
